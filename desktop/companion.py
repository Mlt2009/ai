...
# --- Shared subagent team (same team the mobile web server uses) ---
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from server.orchestrator import build_team, execute_tool
...
if not ELEVENLABS_API_KEY:
    print(">>> [WARN] ELEVENLABS_API_KEY not set — running without spoken TTS output (text only).")
...
class AI_Core(QObject):
    ...
    def __init__(self, video_mode=DEFAULT_MODE):
        ...
        for agent in self.agents.values():
            team_declarations.extend(agent.tool_declarations())

        tools = [{'google_search': {}}, {'code_execution': {}}, {"function_declarations": [create_folder, create_file, edit_file, list_files, read_file, open_application, open_website] + team_declarations}]

        self.config = {
            "response_modalities": ["TEXT"],
            "system_instruction": """
            Your name is Ada and you are my AI companion. You lead a team of
            specialist subagents and use their tools to act in the real world:
            - home__*: smart home control via Home Assistant (lights, climate, scenes, sensors).
            - printer__*: OctoPrint 3D printer control and paper printing via CUPS.
            - computer__*: this computer - live stats, processes, volume, opening apps and sites, shell commands.
            - data__*: real-time data - weather, time, news headlines, crypto prices.
            You also have tools for searching, code execution, and file system actions.
            ...
            1.  For smart home, printer, computer control or real-time data requests, use the matching subagent tool.
            2.  For general information or questions, use `Google Search`.
            3.  For math or running python code, use `code_execution`.
            4.  Use file system functions (`create_folder`, `create_file`, `edit_file`, `list_files`, `read_file`) for any file-related tasks.
            5.  If the user asks to open or launch a desktop application, you must use the `open_application` function.
            6.  If the user asks to open a website or a URL, you must use the `open_website` function.
            Your replies are spoken aloud, so keep them short, natural and conversational.
            If a tool returns an error about missing configuration, tell the user what to add to the .env file.
                ...
                async for chunk in turn:
                    if chunk.tool_call and chunk.tool_call.function_calls:
                        ...
                        for fc in chunk.tool_call.function_calls:
                            args, result = fc.args, {}
                            if "__" in fc.name: result = await execute_tool(self.agents, fc.name, dict(args or {}))
                            elif fc.name == "create_folder": result = self._create_folder(folder_path=args.get("folder_path"))
    ...
    async def tts(self):
        if not ELEVENLABS_API_KEY:
            # No TTS configured — silently drain the queue so turns still complete.
            while self.is_running:
                await self.response_queue_tts.get()
